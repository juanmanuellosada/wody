...
Cada gimnasio tiene un link público `/{slug}/invitarme` que el admin puede compartir por redes sociales, WhatsApp o donde quiera. Desde ese link, los alumnos se registran solos sin que el admin tenga que cargar sus datos a mano.
...
- **Aprobar**: crea el alumno con la contraseña que eligió en el formulario y le manda un mail de bienvenida con el link al login del gimnasio. Ver `docs/emails-resend.md` para el flujo de mails transaccionales.
