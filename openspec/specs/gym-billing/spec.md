...
## Purpose
Cobro mensual del SaaS Wody a cada gym mediante una suscripción única de Mercado Pago ($60.000 ARS/mes vía `preapproval_plan`). Incluye un trial de 30 días desde la creación del gym, exención manual a nivel gym controlada solo por el super-admin, sincronización de estado vía webhook firmado, bloqueo automático por cron diario cuando el trial vence sin suscripción activa, y push notifications a los `ADMIN` en hitos del fin del trial.
