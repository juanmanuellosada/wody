...
- Crear `User` con `role="STUDENT"`, `studentType="GENERAL"`, `accountKind="LITE"`, `canCreateOwnRoutines=false`, `email=null`, `password=null`, `emailVerifiedAt=null`.
...
- **WHEN** el ADMIN abre `UserForm`, ve "Próximo nº de socio (estimado): 45", elige modo `lite`, escribe `name="Carlos"` y envía
- **THEN** se crea un `User` con `email=null`, `password=null`, `accountKind="LITE"`, `role="STUDENT"`, `studentType="GENERAL"`, `canCreateOwnRoutines=false`, `gymId=G`, y un `memberNumber` asignado atómicamente
